...
OBJETOS = sorted([
    ...
    "Contribuições Previdenciárias", "DESCONSTITUTIVA DE AUTOS DE INFRAÇÃO E APREENSÃO C/C ANTECIPAÇÃO DE TUTELA",
...
PEDIDOS_NOME = [
    "Obrigação de Fazer", "Obrigação de Pagar", "Obrigação de Pagar (Ressarcimento SUS)",
...
MATERIA = sorted([
    "Multa", "Multa por Infracao Adminisrativa", "Multa Pecuniária por Infração Administrativa",
...
def normalizar_valores(df: pd.DataFrame) -> pd.DataFrame:
    """Normaliza valores inconsistentes de campos controlados."""
...
def carregar_dados() -> pd.DataFrame:
    if os.path.exists(DATA_PATH):
        df = pd.read_csv(DATA_PATH, index_col=0, dtype=str).fillna("")
    else:
    ...
    df = df.reset_index(drop=True)  # índice sempre contínuo
    return df
...
def inicializar_estado():
    ...
    if "idx_edicao" not in st.session_state:
        st.session_state.idx_edicao = None
...
def opcao_com_outro(lista, label, key, valor_atual=""):
    """Selectbox com opção 'Outro (digitar)'."""
    ...
    if escolha == "✏️ Outro (digitar)":
        return st.text_input(f"{label} (texto livre)", value=valor_atual if valor_atual not in lista else "", key=f"{key}_livre")
    return escolha
...
def renderizar_sidebar():
    with st.sidebar:
        ...
        escolha = st.radio("", paginas, key="nav_radio",
...
def pagina_dashboard():
    ...
    with col_a:
        ...
        status_ct.columns = ["Status", "Quantidade"]
        fig = px.pie(status_ct, names="Status", values="Quantidade",
    ...
    with col_b:
        ...
        prob_ct = df["faixa_de_probabilidade_de_perda_p_a_unimed"].value_counts().reset_index()
        prob_ct.columns = ["Probabilidade", "Quantidade"]
        cores = {"Provável": "#C0392B", "Possível": "#E67E22", "Remota": "#27AE60"}
        fig2 = px.bar(prob_ct, x="Probabilidade", y="Quantidade",
                      color="Probabilidade",
    ...
    with col_c:
        ...
        aba_ct = df["aba"].value_counts().reset_index()
        aba_ct.columns = ["Aba", "Quantidade"]
        fig3 = px.bar(aba_ct, x="Aba", y="Quantidade",
                      color="Aba",
    ...
    with col_d:
        ...
        prov_prob = df.groupby("faixa_de_probabilidade_de_perda_p_a_unimed")[
            "valor_estimado_de_perda_provisao_p_contingencias"
        ].sum().reset_index()
        prov_prob.columns = ["Probabilidade", "Provisão Total"]
        fig4 = px.bar(prov_prob, x="Probabilidade", y="Provisão Total",
                      color="Probabilidade",
                      color_discrete_map={"Provável": "#C0392B", "Possível": "#E67E22", "Remota": "#27AE60"},
                      text_auto=True)
        fig4.update_layout(showlegend=False, margin=dict(t=20, b=20))
        fig4.update_yaxes(tickprefix="R$ ", tickformat=",.0f")
        st.plotly_chart(fig4, use_container_width=True)

    # ── Top 10 maiores provisões ──
...
def formulario_processo(dados: dict = None, prefixo: str = "new") -> dict:
    """Renderiza o formulário de cadastro/edição. Retorna o dict com os valores."""
    d = dados or {}
    ...
    descricao = st.text_area(LABELS["andamentos_descricao"],
                             value=v("andamentos_descricao"),
                             height=140,
...
# ─────────────────────────────────────────────
# PÁGINA 3 – PROCESSOS CADASTRADOS (lista + edição)
# ─────────────────────────────────────────────
...
def pagina_processos():
    ...
    with tab_ed:
        ...
        if salvar_ed:
            for col, val in dados_edit.items():
                st.session_state.df.at[idx_real, col] = val
            salvar_dados(st.session_state.df)
    ...
    with tab_del:
        ...
        if st.button("🗑️ Excluir Processo", type="primary", disabled=not confirmar):
            st.session_state.df = st.session_state.df.drop(index=idx_real).reset_index(drop=True)
            salvar_dados(st.session_state.df)
...
def pagina_relatorio():
    ...
    mask = pd.Series([True] * len(df), index=df.index)
    if r_aba:
        mask &= df["aba"].isin(r_aba)
    if r_status:
        mask &= df["status"].isin(r_status)
    if r_prob:
        mask &= df["faixa_de_probabilidade_de_perda_p_a_unimed"].isin(r_prob)
    if r_risco:
        mask &= df["risco"].isin(r_risco)
    if r_nat:
        mask &= df["natureza"].isin(r_nat)
    if r_cont:
        mask &= df["pedidos_contingencia"].isin(r_cont)
    if r_val_min > 0:
        mask &= df["valor_da_causa_num"] >= r_val_min
    if r_val_max > 0:
        mask &= df["valor_da_causa_num"] <= r_val_max
    ...
    df_exib = df_rel.rename(columns=LABELS)
    ...
    # ── Importar CSV para restaurar dados ──
    st.markdown("---")
    ...
    if uploaded is not None:
        try:
            ...
            if faltando:
                st.error(f"⚠️ O arquivo importado está faltando as colunas: {', '.join(faltando)}")
            else:
                df_import = df_import[COLUNAS]
                df_import = normalizar_valores(df_import)
...
def main():
    # Se a senha NÃO for correta, interrompe a execução aqui
